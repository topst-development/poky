...
if [ "$CMD" = "sstateCache" ]; then
	if [ ! -d $SSTATE_CACHE_REL ]; then
		...
		$AP_addr/sstate_mirror/tcc8050_evb/sstate-cache-rel-topst-development.tar.xz
		...
		tar xvf sstate-cache-rel-topst-development.tar.xz -C build/
		rm -rf sstate-cache-rel-topst-development.tar.xz
